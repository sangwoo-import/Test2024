...
class FreeTalkActivity : AppCompatActivity(), FreeTalkProvider.CallBack {
    ...
    private lateinit var binding: ActivityFreetalkBinding
    private val freeTalkProvider = FreeTalkProvider(this@FreeTalkActivity)

    private lateinit var adapter: FreeTalkRecyclerViewAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityFreetalkBinding.inflate(layoutInflater)
        ...
        recyclerView.layoutManager = LinearLayoutManager(this@FreeTalkActivity)
        // adapter 붙어주기에 만든 data를 붙여주기 + context
        adapter = FreeTalkRecyclerViewAdapter(mutableListOf(), this@FreeTalkActivity)
        recyclerView.adapter = adapter


        /* 들어 가자 마자 실행 */
        val freeTalkRequestDataSet = FreeTalkRequestData(
            keyword, pageSize,
            lastRowNUm
        )
        requestFreeTalk(freeTalkRequestDataSet)
        ...
        binding.FamilyEventSearchBtn.setOnClickListener {
            ...
            // 어댑터 초기화
            adapter.reset()
            val freeTalkRequestData2 = FreeTalkRequestData(
            ...
            requestFreeTalk(freeTalkRequestData2)
        }
        ...
        binding.FamilyEventRecyclerView.addOnScrollListener(object :
            ...
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                when (newState) {
                    RecyclerView.SCROLL_STATE_IDLE -> {    // 젤 상단일 때는 안보이기
                ...
                if (!recyclerView.canScrollVertically(1) && newState == RecyclerView.SCROLL_STATE_IDLE
                    ...
                    if (!isLoading && hasMoreData) {
                        ...
                        lastRowNUm += pageSize
                        val nextPageRequestData = FreeTalkRequestData(
                            keyword = binding.FamilyEventTitleSearchEditText.text.toString(),
                            pageSize,
                            lastRowNUm
                        )
                        requestFreeTalk(nextPageRequestData)
    ...
    private fun requestFreeTalk(freeTalkRequestDataSet: FreeTalkRequestData) {
        freeTalkProvider.FreeTalkGo(freeTalkRequestDataSet)
    }


    override fun completeFreeTalk(code: String, msg: String, resultData: List<FreeTalkResponse>) {
        ...
        if (codeA.equals("100")) {
            ...
            /* 게시글 상세 정보 저장 */
            FreeTalkInformation.saveFreeTalkInfos(boardIdA, postingSnA)
            ...
            Toast.makeText(this@FreeTalkActivity, messageA, Toast.LENGTH_SHORT)
                .show()
